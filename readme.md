...
- 路由方式：`location.hash`（前端根据 hash 动态加载 `pages/*.html` 片段）
...
- `assets/js/components/dashboard.js`：路由与菜单（根据 hash 加载页面片段）
...
1. 点击左侧菜单 → 修改 `location.hash`
2. `assets/js/components/dashboard.js` 监听 hash → `fetch('pages/<route>.html')`
...
- 在 `assets/js/components/dashboard.js` 的 `menuItems` 增加菜单项：`{ label, icon, route: '#<route>' }`。
...
- 路由方式：侧边栏点击修改 `location.hash`；`assets/js/components/dashboard.js` 会 `fetch('pages/<route>.html')` 把 HTML 片段插入 `<main x-ref="mainContainer">`，然后调用 `Alpine.initTree(...)`。
...
- 新增页面必须更新菜单：`assets/js/components/dashboard.js` 的 `menuItems`（`route: '#xxx'`），并确保存在 `pages/xxx.html`。
